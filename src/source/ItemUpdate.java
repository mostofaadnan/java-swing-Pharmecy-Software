 ...
 * @author adnan
 */
public class ItemUpdate extends javax.swing.JInternalFrame {
     ...
     */
    public ItemUpdate() throws IOException, SQLException {
        initComponents();
    ...
    public ItemUpdate(String table_click) throws IOException, SQLException {
        initComponents();
        conn = Java_Connect.conectrDB();
        suppliyer();
    ...
    private void PurchaseNumber() {
        purchasenotext.removeAllItems();
        purchasenotext.addItem("SELECT");
        purchasenotext.setSelectedIndex(0);
        try {
            String sql = "Select purchaseCode from"
                    + " purchase where itemupdate='0' AND GRNstatus='1'";
            pst = conn.prepareStatement(sql);
            rs = pst.executeQuery();
            while (rs.next()) {
                String purchaseCode = rs.getString("purchaseCode");
                purchasenotext.addItem(purchaseCode);
            }
    ...
    private void ItemUpdate() throws SQLException, IOException {
        try {
            ...
            for (int row = 0; row < rows; row++) {
                ...
                try {
                    String sql = "Update Item sta "
                            ...
                            + "sta.mrp='" + mrp + "',"
                            + "sta.batch='" + batch + "' ,"
                            + "sta.expdate='" + expdates + "',"
                            + "sta.boxsize='" + boxsize + "' ,"
                            + "sta.pvat='" + vat + "',"
                            + "sta.pdiscount='" + discount + "',"
                            + "sta.ptpwv='" + tpvate + "',"
                            + "sta.ptpwd='" + tpdiscount + "',"
                            + "sta.smrpwv='" + tpvate + "',"
                            + "sta.pmrpwd='" + tpdiscount + "',"
                            + "sta.profite='" + profitper + "',"
                            + "sta.profiteParcentage='" + profiteParcentage + "' where  sta.Itemcode='" + procode + "'";
                    pst = conn.prepareStatement(sql);
                    pst.execute();
    ...
    private void PurchaseUpdate() throws SQLException {
        try {
            int grnstatus = 1;
            String sql = "Update purchase set itemupdate='" + grnstatus + "' where purchaseCode='" + purchasenotext.getSelectedItem() + "'";
            pst = conn.prepareStatement(sql);
            pst.execute();
            JOptionPane.showMessageDialog(null, "Purchase Item Successfully Update");
        } catch (SQLException | HeadlessException e) {
            JOptionPane.showMessageDialog(null, e);
        }
    }
    ...
    private void Stockpdate() {
        ...
        for (int i = 0; i < dataTable.getRowCount(); i++) {
            ...
            float qty = Float.parseFloat(dataTable.getValueAt(i, 8).toString());
    ...
    private void AfterExecute() {
        ...
        roundingtext.setText(null);
        netpaybletext.setText(null);
        remarktext.setText(null);
        PurchaseNumber();
    }
    ...
    private void purchasenotextPopupMenuWillBecomeInvisible(javax.swing.event.PopupMenuEvent evt) {//GEN-FIRST:event_purchasenotextPopupMenuWillBecomeInvisible
        if (purchasenotext.getSelectedIndex() > 0) {
            String SearchText = (String) purchasenotext.getSelectedItem();
            try {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        DefaultTableModel model2 = (DefaultTableModel) dataTable.getModel();
        if (model2.getRowCount() > 0 && purchasenotext.getSelectedIndex() > 0) {
